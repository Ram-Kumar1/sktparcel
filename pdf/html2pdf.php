...
$pdf->address('address');
$pdf->PutMinorHeadingTime("Date = ".@date("F j, Y",$this->date));
$pdf->PutMinorHeading('To');
$pdf->address('Arulmigu venkatesh perumal kovil');
$pdf->PutMinorHeading('Dear Sir, We are pleased to supply our products with the following price details.');
$pdf->PutMinorHeadingTerms('Terms and Conditions:');
$pdf->terms('18% GST extra for street light poles ');
$pdf->terms('Delivery based on quantity');
$pdf->terms('Transportation costs extra');
$pdf->terms('payment Terms:50% on purchase order.Balance before delivery of goods.');
$pdf->Ln(4);
$pdf->terms('Trust on offer is in-line with your requirement and if you require any further clarification in this regard,');
$pdf->terms('please feel free to contact us.');
$pdf->terms('Thanking you and assuring you of our best and personalized services at all times and looking forward');
$pdf->terms('your valuable order.');
...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$this->SetStyle('B',true);
		$this->Ln(12);
		$this->Cell(0,0,'SUNMAC ENTERPRISES',0,0,'C');
		$this->Ln(2);
		$this->SetFontSize(12);
		$this->SetFont('Times','',12);
		$this->mySetTextColor(-1);
	...
	function PutMinorHeadingTerms($title) {
		$this->SetFontSize(12);
		$this->SetStyle('U',true);
		$this->Cell(0,5,$title,0,1,'L');
		$this->SetFontSize(12);
	...
	function PutMinorHeadingTime($title) {
		$this->SetFontSize(12);
		$this->SetStyle('B',true);
